	...
	public function buildConsumeSeedElementBlock(cases:MacroArpSwitchBlock):Void {
		if (this.isSeedableAsGroup) {
			...
			caseBlock.push(macro @:pos(this.nativePos) {
				for (e in element) this.$i_nativeName.slots.set(element.keyOrAuto(autoKey), this._arpDomain.loadSeed(e, ${this.eArpType}).addReference());
			});
		}
		if (this.isSeedableAsElement) {
			...
			caseBlock.push(macro @:pos(this.nativePos) {
				this.$i_nativeName.slots.set(element.keyOrAuto(autoKey), this._arpDomain.loadSeed(element, ${this.eArpType}).addReference());
			});
		}
	}
